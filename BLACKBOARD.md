...
- 2026-05-22 routing/policy status: Skill Builder now defaults back to `blueprint only`; the earlier three CSV companion docs were compressed into the exception-only pair `boards/SkillBluePrint/skill-csv-exception-guide.md` and `skill-builder-handoff-format.md`, and `AGENTS_ROLE/GAMEBULIDER_SKILL.md` / `MDTREE.md` now open them only when blueprint-only work cannot safely continue. Detailed record is in `boards/OPS/AUTOMATION_GUIDE.md`.
