...
namespace Pakuri.NewCore.Presentation.Actors
{
    public abstract class UnitActorBehaviour : MonoBehaviour
    {
        ...
        private sealed class UnitWorldView
        {
            ...
            public IEnumerator ShowDamage(float amount)
            {
                if (damageLabel == null)
                {
                    yield break;
                ...
                damageLabel.text = Format(amount);
                damageLabel.gameObject.SetActive(true);
                var color = damageLabel.color;
                color.a = 1f;
                ...
                while (elapsed < 0.6f)
                {
                    ...
                    color.a = 1f - Mathf.Clamp01(elapsed / 0.6f);
                    damageLabel.color = color;
                    yield return null;
                }
