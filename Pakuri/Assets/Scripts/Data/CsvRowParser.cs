...
namespace Pakuri.Data
{
    internal static class CsvRowParser
    {
        ...
        internal static SkillChoiceRow ParseSkillChoiceRow(CsvRecord record /* 읽을 CSV 행 */)
        {
            ...
            if (row.HasShotIntervalMultiplier)
            {
            ...
            if (row.HasBurstDamageMultiplier)
            {
            ...
            row.FollowUpProjectileDamageMultiplier = ReadOptionalFloat(record, "follow_up_projectile_damage_multiplier", 1f);
            row.HasReloadTimeMultiplier = TryReadFloatIfColumnExists(record, "reload_time_multiplier", out var reloadTimeMultiplier);
            if (row.HasReloadTimeMultiplier)
            {
            ...
            if (row.HasKnockbackDistanceMultiplier)
            {
            ...
            if (row.HasDamageDelayMultiplier)
            {
            ...
            if (row.HasBranchDamageMultiplier)
            {
            ...
            if (row.HasBossDamageMultiplier)
            {
            ...
            if (row.HasConditionalDamageMultiplier)
            {
            ...
            if (row.HasTargetStatusStackDamageMultiplier)
            {
            ...
            row.CountTargetSide = ReadOptionalEnum(
                record,
                "count_target_side",
            ...
            row.OnHitAdditionalDamageMultiplier = ReadOptionalFloat(record, "on_hit_additional_damage_multiplier", 1f);
            row.OnHitAdditionalDamageAttribute = ReadOptionalEnum(record, "on_hit_additional_damage_attribute", DamageAttribute.Physical);
            ...
            row.OnHitChainDamageMultiplier = ReadOptionalFloat(record, "on_hit_chain_damage_multiplier", 1f);
            row.OnHitChainDamageAttribute = ReadOptionalEnum(record, "on_hit_chain_damage_attribute", DamageAttribute.Physical);
            ...
            if (row.HasCoreDamageMultiplier)
            {
            ...
            row.CoreOnHitAdditionalDamageMultiplier = ReadOptionalFloat(record, "core_on_hit_additional_damage_multiplier", 1f);
            row.CoreOnHitAdditionalDamageAttribute = ReadOptionalEnum(record, "core_on_hit_additional_damage_attribute", DamageAttribute.Physical);
            ...
            row.RepeatDamageMultiplier = ReadOptionalFloat(record, "repeat_damage_multiplier", 1f);
            return row;
        }
