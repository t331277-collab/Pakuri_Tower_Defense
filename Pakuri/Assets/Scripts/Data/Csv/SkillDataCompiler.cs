...
public static class SkillRuntimeCompiler
{
	...
	public static PassiveSkillRuntimeData CompilePassive(MonsterDefinition monster, PassiveDefinition source)
	{
		...
		StatusDataCompiler.CompileSkillEffects(passiveSkillRuntimeData.MultiEffects);
		SkillTriggerDefinition[] triggers = null;
		if (monster != null)
		{
			triggers = monster.SkillTriggers;
		...
		StatusDataCompiler.CompileTriggers(passiveSkillRuntimeData.SkillTriggers);
	...
	private static void MapCommonFields(SkillRuntimeData skill, string monsterId, SkillDefinition source, SkillTriggerDefinition[] monsterTriggers = null)
	{
		...
		StatusDataCompiler.CompileSkillEffects(skill.MultiEffects);
		skill.SkillTriggers = FilterSkillTriggersForSkill(monsterTriggers, source.SkillId);
		StatusDataCompiler.CompileTriggers(skill.SkillTriggers);
		...
		{
			skill.Targeting.SelectionStatusKind = StatusDataCompiler.ParseStatusKind(
	...
	private static void MapActiveFields(SkillRuntimeData skill, MonsterDefinition monster, SkillDefinition source)
	{
		...
		else if (skill is SingleSkillRuntimeData singleSkillRuntimeData)
		{
			...
			{
				singleSkillRuntimeData.DeploymentRequiredTargetStatusKind = StatusDataCompiler.ParseStatusKind(
			...
			{
				singleSkillRuntimeData.TargetStatusStackStatusKind = StatusDataCompiler.ParseStatusKind(
			...
			{
				singleSkillRuntimeData.ConsumeTargetStatusKind = StatusDataCompiler.ParseStatusKind(
		...
		if (!StatusDataCompiler.TryParseShieldRefreshRule(source.ShieldAmountRefreshPolicy, out rule))
			{
	...
	{
		string value = source.StatusEffectLabel;
		if (!string.IsNullOrWhiteSpace(source.StatusEffectId))
		{
			value = source.StatusEffectId.Trim();
		}
		if (string.IsNullOrWhiteSpace(value))
		{
			return null;
		}
		StatusEffectKind kind = StatusDataCompiler.ParseStatusKind(value);
		StatusRuntimeData runtimeStatusData = StatusDataCompiler.Create(kind, source.StatusEffectLabel, source);
	...
	private static BuffTarget MapBuffTarget(SkillDefinition source)
	{
		StatusDataCompiler.TryParseTargetScope(source.StatusTargetScope, out var scope);
...
namespace Pakuri.InGame
{
	...
	private static SkillActionOp? MapSkillActionOp(SkillNodeDefinition node, string handlerId)
	{
		...
		if (string.Equals(handlerId, "CountStatusDamageMultiplier", StringComparison.OrdinalIgnoreCase))
		{
			...
			StatusEffectKind statusKind = StatusDataCompiler.ParseStatusKind(statusId);
		...
		if (string.Equals(handlerId, "StatusConditionalDamageTakenBonus", StringComparison.OrdinalIgnoreCase))
		{
			...
			return new SkillActionOp(
				...
				0f,
				0f,
				StatusDataCompiler.ParseStatusKind(sourceStatusId));
		}
...
namespace Pakuri.InGame
{
	...
	internal static SkillChoiceRuntimeData[] Compile(SkillChoiceDefinition[] source)
	{
		...
		for (int i = 0; i < source.Length; i++)
		{
			...
			{
				skillChoiceDefinition.CountStatusKind = StatusDataCompiler.ParseStatusKind(
					skillChoiceDefinition.CountStatusId);
			}
			if (!string.IsNullOrWhiteSpace(skillChoiceDefinition.ConditionalCritTargetStatusId))
			{
				skillChoiceDefinition.ConditionalCritTargetStatusKind = StatusDataCompiler.ParseStatusKind(
			...
			{
				skillChoiceDefinition.RequiredSourceStatusKind = StatusDataCompiler.ParseStatusKind(
	...
	private static void ApplyNormalizedChoiceNode(SkillChoiceRuntimeData spec, SkillNodeDefinition node)
	{
		...
		else if (string.Equals(a, "CountStatusDamageMultiplier", StringComparison.OrdinalIgnoreCase))
		{
			...
			source.CountStatusKind = StatusDataCompiler.ParseStatusKind(source.CountStatusId);
		...
		else if (string.Equals(a, "ThresholdApplyStatus", StringComparison.OrdinalIgnoreCase))
		{
			...
			source.ThresholdStatusKind = StatusDataCompiler.ParseStatusKind(source.ThresholdStatusId);
			...
			source.ThresholdApplyStatusKind = StatusDataCompiler.ParseStatusKind(source.ThresholdApplyStatusId);
		}
		...
		else if (string.Equals(a, "StatusConditionalDamageTakenBonus", StringComparison.OrdinalIgnoreCase))
		{
			...
			source.StatusConditionalSourceStatusKind = StatusDataCompiler.ParseStatusKind(
		...
		else if (string.Equals(a, "TargetStatusCritBonus", StringComparison.OrdinalIgnoreCase))
		{
			...
			source.ConditionalCritTargetStatusKind = StatusDataCompiler.ParseStatusKind(
		...
		else if (string.Equals(a, "RedistributeConsumedStatus", StringComparison.OrdinalIgnoreCase))
		{
			...
			source.RedistributeConsumedStatusKind = StatusDataCompiler.ParseStatusKind(
