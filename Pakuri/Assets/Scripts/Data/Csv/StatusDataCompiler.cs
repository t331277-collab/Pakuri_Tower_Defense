...
{
    public static class StatusDataCompiler
    {
        public static void CompileSkillEffects(SkillEffectDefinition[] effects)
        {
            ...
            for (var i = 0; i < effects.Length; i++)
            {
                ...
                {
                    effect.RequiredSourceStatusKind = ParseStatusKind(effect.RequiredSourceStatusId);
                }
                ...
                var statusId = effect.StatusEffectId;
                if (string.IsNullOrWhiteSpace(statusId))
                {
                    statusId = effect.StatusEffectLabel;
                }

                effect.StatusKind = ParseStatusKind(statusId);
                if (effect.EffectKind == SkillMultiEffectKind.Status)
                {
        ...
        public static StatusRuntimeData Create(StatusEffectKind kind, string label, SkillDefinition source)
        {
            ...
            if (source.RuntimeKind == SkillRuntimeKind.Buff || source.RuntimeKind == SkillRuntimeKind.Shield)
            {
        ...
        {
            var statusKey = effect.StatusEffectId;
            if (string.IsNullOrWhiteSpace(statusKey))
            {
                statusKey = effect.StatusEffectLabel;
            }

            var kind = ParseStatusKind(statusKey);
            var status = Create(kind, effect.StatusEffectLabel);
            status.SourceSkillId = effect.EffectId;
            if (string.IsNullOrWhiteSpace(status.SourceSkillId))
            {
                status.SourceSkillId = effect.SkillId;
            }
            ...
            {
                status.TargetScope = ParseTargetScope(effect.StatusTargetScope);
            }

            if (!string.IsNullOrWhiteSpace(effect.StatusMergePolicy))
            {
                status.MergePolicy = ParseMergePolicy(effect.StatusMergePolicy);
            }

            if (!string.IsNullOrWhiteSpace(effect.ShieldAmountRefreshPolicy))
            {
                status.ShieldAmountRefreshPolicy = ParseShieldRefreshRule(effect.ShieldAmountRefreshPolicy);
            }
            ...
            if (effect.StatusMoveSpeedBonus < 0f)
            {
                status.MovementSlowRate = -effect.StatusMoveSpeedBonus;
            }
            else
            {
                status.MovementSlowRate = 0f;
            ...
            if (!string.IsNullOrWhiteSpace(effect.StatusConditionalTargetStatusId))
            {
                status.ConditionalTargetStatusKind = ParseStatusKind(effect.StatusConditionalTargetStatusId);
            }
            ...
            status.ConditionalIncomingSkillRuntimeKindValues = ParseSkillRuntimeKindConditions(
                effect.StatusConditionalIncomingSkillRuntimeKinds);
            status.ConditionalOutgoingSkillRuntimeKinds = effect.StatusConditionalOutgoingSkillRuntimeKinds;
            status.ConditionalOutgoingSkillRuntimeKindValues = ParseSkillRuntimeKindConditions(
                effect.StatusConditionalOutgoingSkillRuntimeKinds);
            ...
            status.ElementModifierTarget = effect.Attribute;
            status.Modifiers.ResistReduction = status.ElementResistReduction;
