    ...
    public class SpawnManager : MonoBehaviour
    {
            ...
            IReadOnlyList<StageEncounterDefinition> encounterRows)
        {
                ...
                rowIndex++)
            {
                ...
                int count = row.count.Value;
                float interval = row.interval_sec.Value;
        ...
        private void Spawn(SpawnEntry entry)
        {
            ...
            EnemyModel model = new EnemyModel(
                ...
                multiplier);
            float x = entry.Encounter.spawn_x.Value;
            float yMinimum = entry.Encounter.spawn_y_min.Value;
            float yMaximum = entry.Encounter.spawn_y_max.Value;
        ...
        private float ResolveBossHealthMultiplier(
            StageEncounterDefinition encounter)
        {
            float minimum = encounter.boss_health_multiplier_min.Value;
            float maximum = encounter.boss_health_multiplier_max.Value;
        ...
        public MonsterActor EnsureMonster(MonsterModel model)
        {
            ...
            runtime.RegisterMonster(model, slot == 0);
            return actor;
        }
        ...
        private GameObject ResolveMonsterPrefab(string monsterId)
        {
            switch (monsterId)
            {
                case "ariel": return arielUnitPrefab;
                case "eve": return eveUnitPrefab;
                case "rin": return rinUnitPrefab;
                case "sein": return seinUnitPrefab;
                case "vega": return vegaUnitPrefab;
                default:
                    break;
            }
