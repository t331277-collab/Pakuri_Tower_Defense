    ...
    public sealed class NewCoreInGameUIController : MonoBehaviour
    {
            ...
            Units.Models.MonsterModel monster)
        {
            ...
            for (var index = 0; index < offeringButtons.Length; index++)
            {
                ...
                var ownerDefinition = activeOffer.Monster.MonsterDefinition;
                var ownerName = string.IsNullOrWhiteSpace(
                    ownerDefinition.display_name)
                        ? ownerDefinition.id
                        : ownerDefinition.display_name;
                BindOfferingCandidate(button, candidate, ownerName);
                button.onClick.AddListener(
        ...
        private static void BindOfferingCandidate(
            Button button,
            ...
            string ownerName)
        {
            SetChildText(
                button.transform,
                "SkillName",
                ResolveOfferingLabel(candidate));
            SetChildText(
                button.transform,
                "Summary",
                ownerName);
            SetChildText(
                button.transform,
                "Desc",
                ResolveOfferingDescription(candidate));
        }
        ...
        private static string ResolveOfferingDescription(
            OfferingCandidate candidate)
        {
