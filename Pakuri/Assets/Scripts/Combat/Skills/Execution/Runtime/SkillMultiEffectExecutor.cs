    ...
    internal static class SkillMultiEffectExecutor
    {
            ...
            Vector2 fallbackCenter)
        {
            ...
            if (explicitTarget != null)
            {
                ...
                if (effects != null)
                {
                    effects.SpawnEffectVisual(effect, center, 1f);
                }
                return true;
            }

            var routed = ZoneSkillActor.ApplyAreaTick(
                context.CombatManager,
                context.CasterEntry,
                context.Roster,
                targeting,
            ...
            out bool routed)
        {
            ...
            var instance = context.CombatManager.Effects.SpawnAreaEffect(
                runtimeVisual,
                null,
            ...
            bool scaleStatusDurationWithSnapshot)
        {
            ...
            if (routed)
            {
                if (visualTargets != null)
                {
                    ...
                    if (effects != null)
                    {
                        effects.SpawnEffectVisualOnTargets(
                            effect,
                            visualTargets,
                            statusSpec.DurationSeconds);
                    }
                ...
                else
                {
                    ...
                    if (effects != null)
                    {
                        effects.SpawnEffectVisual(
                            effect,
                            ResolveEffectCenter(context, effect, targeting, fallbackCenter),
                            1f);
                    }
                }
            }
            ...
            ProjectileStatusHitSpec statusSpec)
        {
            ...
            var instance = context.CombatManager.Effects.CreateAreaEffectObject(
                effect.RuntimeVisual,
                effect.SkillEffectPrefab,
                string.IsNullOrWhiteSpace(effect.EffectId)
                    ? "SkillEffectZone"
                    : $"SkillEffectZone_{effect.EffectId}",
                center,
                effect.Radius,
                snapshot,
