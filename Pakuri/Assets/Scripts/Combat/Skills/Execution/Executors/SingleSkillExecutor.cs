...
namespace Pakuri.InGame
{
    ...
    public sealed class SingleSkillExecutor : TypedSkillExecutor<SingleSkillRuntimeData>
    {
        ...
        private readonly struct SingleFollowUpTarget
        {
             ...
             */
            public SingleFollowUpTarget(BaseUnitRuntimeModel model, Vector2 center)
            {
                Model = model;
                Center = center;
            }
        ...
        {
            var skill = context != null ? context.SkillRuntimeData as SingleSkillRuntimeData : null;
            ...
            if (SingleSkillRuleHandlers.ShouldRejectCastForExecuteThreshold(context, snapshot, skill))
            {
            ...
            var prefab = hasRuntimeVisual || effects == null
                ? null
                : effects.ResolveSkillEffectPrefab(
                    context.Caster,
                    skill.SkillId,
                    preferredPrefab);
         ...
         * 반경을 결정한다.
         */
        private static float ResolveRadius(SingleSkillRuntimeData skill, SkillExecutionSnapshot snapshot)
        {
         ...
         * 배치 횟수를 결정한다.
         */
        private static int ResolveDeploymentCount(SingleSkillRuntimeData skill, SkillExecutionSnapshot snapshot)
        {
         ...
         * 실제 적중 대상 횟수를 결정한다.
         */
        private static int ResolveEffectiveHitTargetCount(SingleSkillRuntimeData skill, SkillExecutionSnapshot snapshot)
        {
            if (skill == null)
            {
                return 1;
         ...
         * 결정된 배치를 실행한다.
         */
        private static SingleExecutionOutcome ExecuteResolvedDeployments(
            SkillExecutionContext context,
            ...
            GameObject prefab)
        {
            ...
            return new SingleExecutionOutcome(routed, castCommitted);
        }
         ...
         */
        private static SingleExecutionOutcome ExecuteAtCenter(
            SkillExecutionContext context,
            SkillExecutionSnapshot snapshot,
            SingleSkillRuntimeData skill,
            Vector2 center,
            ...
            bool allowConditionalFollowUp)
        {
            ...
            var followUpTargets = followUpSpec.HasValue ? new List<SingleFollowUpTarget>() : null;
            ...
            if (skill.UsePrefabHitbox && (hasRuntimeVisual || prefab != null) && effects != null)
            {
                ...
                if (instance != null)
                {
                    ...
                    if (EffectVisualUtility.UsesSingleLineVisual(skill))
                    {
                        effects.ConfigureSingleLineEffect(
                            instance,
                            context,
                            skill,
                            snapshot,
                    ...
                    effects.DestroyAfterAnimation(
                        instance,
                        damageDelaySeconds + PostDamageLifetimePaddingSeconds,
                        DefaultVisualLifetimeSeconds);
                }
            }
                    ...
                    if (routed)
                    {
                        if (effects != null)
                        {
                            effects.SpawnAnimatedEffect(
                                ...
                                center,
                                Quaternion.identity,
                                PostDamageLifetimePaddingSeconds,
                                DefaultVisualLifetimeSeconds);
                        }
                    }
                }
        ...
        private static bool ApplyNonPrefabTargets(
            ...
            SingleFollowUpSpec? followUpSpec,
            List<SingleFollowUpTarget> followUpTargets)
        {
            ...
            List<SingleFollowUpTarget> followUpTargets)
        {
            ...
            for (var i = 0; i < targets.Count; i++)
            {
                ...
                SingleSkillRuleHandlers.HandleKillRecovery(sourceRuntime, skill, snapshot, result, targetDamage.IsExecute);
         ...
         * 후속 설정을 결정한다.
         */
        private static SingleFollowUpSpec? ResolveFollowUpSpec(
            SkillExecutionSnapshot snapshot,
            ProjectileStatusHitSpec statusSpec,
            GameObject prefab)
        {
        ...
        private static IEnumerator ExecuteConditionalFollowUpAfterDelay(
            ...
            SkillExecutionSnapshot snapshot,
            SingleSkillRuntimeData skill,
            SingleFollowUpTarget followUpTarget,
            SingleFollowUpSpec followUpSpec,
            float delaySeconds)
        {
            yield return new WaitForSeconds(Mathf.Max(0f, delaySeconds));
            ...
            bool isCoreHit)
        {
            ...
            var modifierState = SingleSkillRuleHandlers.ApplyDamageModifiers(skill, snapshot, target, damageMultiplier, critChanceBonus);
