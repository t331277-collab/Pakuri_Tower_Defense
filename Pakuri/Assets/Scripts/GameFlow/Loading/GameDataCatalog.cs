    ...
    class GameDataLookup
    {
        ...
        private void RegisterMonsters(MonsterDefinition[] catalogMonsters)
        {
            ...
            for (var i = 0; i < catalogMonsters.Length; i++)
            {
                ...
                var monsterPassiveSkills = monster.PassiveSkills;
                if (monsterPassiveSkills == null)
                {
                    monsterPassiveSkills = Array.Empty<PassiveDefinition>();
                }
                passiveSkillsByMonster[monster.MonsterId] = monsterPassiveSkills;

                var monsterRewardChoices = monster.InitialRewardChoices;
                if (monsterRewardChoices == null)
                {
                    monsterRewardChoices = Array.Empty<MonsterDefinition.RewardChoiceDefinition>();
                }
                rewardChoicesByMonster[monster.MonsterId] = monsterRewardChoices;
