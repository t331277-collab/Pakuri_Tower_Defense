...
namespace Pakuri.InGame
{
    public class EnemyCombatController
    {
        ...
        private void TickEnemy(
            ...
            float deltaTime)
        {
            if (!EnemyTargetSelector.IsActive(enemyEntry))
            {
                return;
            }
            ...
            if (SingleChargeRuntime.Tick(enemyEntry, roster, combatManager, deltaTime))
            {
                return;
            }

            var target = EnemyTargetSelector.FindNearestPlayerTarget(enemyEntry, roster);
            // 일반 플레이어가 모두 사라진 뒤 선택된 넥서스는 별도 접촉 공격으로 처리한다.
            if (target != null && EnemyTargetSelector.IsNexus(target))
            {
        ...
        private bool CanExecuteSupportSkill(SkillRuntimeInstance runtime)
        {
            ...
            return !(runtime.Data is BuffHealSkillRuntimeData)
                || EnemyTargetSelector.FindLowestHealthEnemyAlly(roster) != null;
        }
