...
- Goals: `Artifact_Container/Artifact` 클릭으로 현재 컨테이너 시너지 팝업을 열고, 같은 시너지 유물 10종의 아이콘·보유 색상·클릭 설명과 2/4/6/8 단계 활성 색상·클릭 설명을 표시한다. 팝업·시너지 상세·유물 상세는 스택에 쌓고 외부 클릭마다 마지막 패널 하나만 닫는다. 상세 패널의 최신 출력 계약은 `Info=설명`, `Text (TMP)=이름 또는 {숫자} 시너지`다. 복제된 모든 `Artifact_Container`도 같은 동작을 사용한다.
- Constraints: 기존 `InGameInfoUI`의 시너지별 그룹화와 `-93.3` 복제 배치, `GameDataLoader.CurrentCatalog`, `RunSession.PartyMembers[*].Artifacts.OwnedArtifactNames`, 상대 경로 런타임 바인딩을 재사용한다. hover 또는 매 프레임 마우스 감지를 사용하지 않는다. 신규 데이터 스키마·Inspector 오브젝트 참조·시너지별 전용 팝업을 만들지 않는다. 외부 클릭은 투명 런타임 차단면이 받아 게임/UI로 관통시키지 않는다. 사용자 Play Mode 검증은 사용자 소유다.
- Role Owner: Code Builder.
- Status: Artifact Button 클릭 전용 활성화, 매 프레임 강제 종료, 복제 컨테이너 가림 결함 수정 완료. `Artifact_Container`를 `UI/Reward` 마지막 자식으로 이동하고 런타임 바인딩 경로를 갱신했다. 복제본은 Reward 전체 첫 자리가 아니라 원본 바로 앞에 역순으로 삽입하도록 수정했다. 빌드·Unity 스크립트/씬 진단 통과; 사용자 Play Mode 확인 대기.
- Next Actions: Play Mode에서 복제 순서가 `Artifact_Container (N) ... Artifact_Container (1) → Artifact_Container`인지, 원본과 복제 컨테이너의 `Artifact` 클릭 열기, hover만으로는 열리지 않음, 열린 상태 유지, `Info=설명`·`Text (TMP)=이름/시너지 숫자`, 상세 패널 LIFO 외부 클릭 닫기, `UI/Reward` 하위 이동 후 표시/팝업 정렬을 확인한다.
- Evidence: `GetArtifactContainer()` 243~248행은 새 복제본을 `artifactContainerTemplate.GetSiblingIndex() - (artifactContainers.Count - 1)` 위치에 삽입한다. 첫 복제본은 원본 바로 앞, 다음 복제본은 기존 복제본보다 앞에 놓여 `Artifact_Container (N) ... Artifact_Container (1) → Artifact_Container` 순서를 유지한다. `InGameInfoUI.BindObject()` 135~139행은 `Reward/Artifact_Container`를 바인딩한다. Unity MCP 씬 계층에서 `UI/Reward` 자식 순서는 `ArtifactPanel`, `RewardPanel`, `PrisonPanel`, `OfferingPanel`, `Artifact_Container`이며 `Artifact_Container`가 마지막이다. 이동 후 로컬 위치는 `{x:0,y:0,z:0}`이다. 라이브 씬과 `InGameScene.unity`에서 `UI/Reward/Artifact_Container/Artifact`는 Image와 Button을 가지며 Button은 interactable, targetGraphic은 같은 Artifact Image, persistent onClick은 비어 있다. `BindArtifactPopupInteractions()`은 이 `PopupOpenButton.onClick`에 `OpenArtifactPopup`을 등록하며 Artifact에 EventTrigger를 생성하지 않는다. EventTrigger는 런타임 외부 클릭 차단면에만 남는다. `OpenArtifactInfo()` 658~659행은 `ArtifactInfo.Info`에 `artifact.Description`, `ArtifactInfo.Text (TMP)`에 `artifact.DisplayName`을 넣고, `OpenSynergyInfo()` 674~675행은 `SynergyInfo.Info`에 `level.Description`, `SynergyInfo.Text (TMP)`에 `$"{level.RequiredCount} 시너지"`를 넣는다. 솔루션 빌드 오류 0·기존 참조 경고 2개, Unity `validate_script` diagnostics 0, 씬 `validate` issues 0이다. 콘솔의 단일 오류는 프로젝트 코드가 아닌 `com.coplaydev.unity-mcp`의 disposed `NetworkStream` 전송 오류다.
- History: 2026-08-09 Designer가 사용자 작성 중인 `InGameScene.unity`와 `NanumGothic SDF.asset` 변경을 보존한 채 실제 씬·코드·CSV·Unity 상태를 검사했다. 2026-08-09 사용자가 기존 pointer-exit 설계를 외부 클릭 LIFO 닫기로 대체했다. Code Builder는 새 MonoBehaviour 없이 `InGameInfoUI`에 최소 바인딩과 전역 스택을 추가하고, 원본을 복제한 컨테이너도 동일 경로로 재바인딩했다. 유물 순서는 catalog 순서를 쓰며 `tracker`는 icon·설명·유물이 없어 적용 대상에서 제외했다. 2026-08-09 사용자 Play Mode 제보 뒤 Designer가 매 프레임 Refresh와 스택 초기화의 충돌, 클릭 open 미등록을 결함 원인으로 확정했다. 이어 Code Builder가 hover를 클릭으로 교체하고 팝업 상호작용 상태를 매 프레임 데이터 Refresh에서 분리했다. 사용자가 Artifact에 Button을 저장한 뒤 Code Builder가 Artifact open 입력을 EventTrigger에서 해당 Button.onClick으로 교체했다. 같은 날 사용자 확인에 따라 두 상세 패널의 Info/Text 출력 대입을 모두 반전했다. 복제 팝업 가림 제보 뒤 Code Builder가 복제 직후 `SetAsFirstSibling()`을 추가했다. 같은 날 Code Builder가 `Artifact_Container`를 `UI/Reward` 마지막 자식으로 재부모화하고 `InGameInfoUI` 경로를 `Reward/Artifact_Container`로 변경했다. 이후 Reward 최상단으로 올라간 복제본 제보에 따라 원본 직전 삽입 방식으로 변경했다.
