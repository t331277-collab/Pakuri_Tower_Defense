...
## Task: 2026-05-17 NewMainMenu Legacy Flow Removal Phase5-6

### Task title

Finalize the UI-side old scene cleanup so only the new menu/run scene pair remains.

### Goals

- Leave `NewMainMenu.unity` and `NewRunScene.unity` as the only surviving scene files.
- Remove old menu/run/debug scenes and the old UI/controller scripts they depended on.
- Preserve the current `UIManager` -> `NewRunStartContext` -> `NewRunScene` flow.

### Constraints

- Role Owner is Code Builder.
- No UI layout, binding logic, or `NewScene` hierarchy editing was intentionally changed in this slice.
- `NewRunScene` still keeps one Legacy data-asset reference (`GameDataCatalog.asset`) and that asset was not deleted here.
...
UI-side old-scene cleanup completed; only the new scene pair remains.

### Next Actions

- If the user wants a fully non-Legacy asset path, replace the kept `GameDataCatalog.asset` reference inside the new flow before deleting remaining Legacy data assets.
- User verifies in Unity that the supported entry flow is now only `Assets/Scenes/NewScene/NewMainMenu.unity` -> `Assets/Scenes/NewScene/NewRunScene.unity`.

### Evidence

- `Get-ChildItem Pakuri\Assets\Scenes -Recurse -Filter *.unity` now returns only `NewScene/NewMainMenu.unity` and `NewScene/NewRunScene.unity`.
- `Pakuri/ProjectSettings/EditorBuildSettings.asset` now lists only those two `NewScene` paths.
- `Pakuri/Assets/Scripts2/UI/UIManager.cs` was not redirected; it still uses `NewRunStartContext.Prepare(monsterId)` and the new run scene path.
- Deleted old UI/controller scripts together with the `Pakuri/Assets/Legacy/Scripts` tree, including `MainMenuFlowController.cs`, `RunCombatUiController.cs`, and `DebugSceneController.cs`.
- Unity console warning/error read after refresh showed only MCP client handler logs, not project UI/scene errors.

### History

- 2026-05-17: Code Builder completed the final scene/controller cleanup under the user's instruction to keep only `Assets/Scenes/NewScene` scenes.
