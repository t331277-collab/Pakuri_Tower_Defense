...
## Task: 2026-05-18 NewRunScene EffectManager Skill Visual Authority

### Task title

Move `NewRunScene` monster/enemy skill visual prefab resolution out of `InGameCombatManager` and into `EffectManager`.

### Goals

- Keep `NewRunScene` skill visual spawning on a dedicated scene component instead of scattered combat-manager fields or CSV prefab-path ownership.
- Preserve current runtime skill instantiation behavior while making inspector editing easier by grouping entries per unit.
- Keep `RunTimeSkill` as the spawned effect parent transform for runtime scene cleanup/readability.

### Constraints

- Role Owner is Code Builder.
- This slice changed skill visual authority only; it did not move enemy passive numeric logic out of `StageOneEnemyPassiveStatApplier.cs`.
- Current monster skill choice effect overrides still come from runtime choice data and can bypass `EffectManager` through `SkillExecutionSnapshot.SkillEffectPrefab`.
...
- If the user wants `InGameCombatManager` to narrow further, the next candidate split is projectile boundary / projectile lifecycle ownership.
- If future monster skills need scene-authored effect variants by enhancement choice, decide whether those should stay as snapshot overrides or move to additional `EffectManager` group types.
- If future stage scenes need different effect bindings, treat `EffectManager` as the scene-local override point instead of reintroducing combat-manager switch fields.

### Evidence

- Added `Pakuri/Assets/Scripts2/InGame/Core/EffectManager.cs` with grouped `monsterSkillEffects` and `enemySkillEffects` registries plus `InstantiateSkillPrefab(...)`.
- `Pakuri/Assets/Scripts2/InGame/Core/InGameCombatManager.cs` now exposes `Effects => ResolveEffectManager()` and no longer declares the old serialized monster skill effect prefab fields.
- `Pakuri/Assets/Scripts2/InGame/Core/EnemyCombatSimulationSystem.cs` now resolves enemy effect prefabs through `combatManager.Effects.ResolveEnemySkillEffectPrefab(...)`.
- `Pakuri/Assets/Scripts2/InGame/Skills/Execution/SkillExecutors.cs` now resolves base monster effect prefabs through `combatManager.Effects.ResolveMonsterSkillEffectPrefab(context.Caster, skill.SkillId)`.
- `Pakuri/Assets/Scripts2/InGame/Skills/Execution/InGameProjectileActor.cs` now spawns branch projectiles through `combatManager.Effects.InstantiateSkillPrefab(...)`.
- Unity-MCP saved `Assets/Scenes/NewScene/NewRunScene.unity` with `EffectManager` attached to `GameManager`, `runtimeSkillRoot=RunTimeSkill`, `monsterGroups=5`, and `enemyGroups=8`.
- Unity-MCP validation reported `NewRunScene` clean with `missingScripts=0`, `brokenPrefabs=0`, and no scene issues.
- Runtime/editor builds completed with 0 errors and only the existing assembly-reference warnings.
- After the follow-up CSV fix, Unity-MCP reimported `Assets/CSVdata/EnemySkillData.csv` and `PakuriCsvRuntimeData.SyncImportedSourceCatalogsForEditor()` succeeded with `monsters=5` and `enemies=8`.

### History

- 2026-05-18: User explicitly assigned Code Builder to implement a new `EffectManager`, move skill-effect management out of `InGameCombatManager`, and make `NewRunScene` inspector data the single source of truth for monster/enemy skill effect prefabs.
- 2026-05-18: User then reported a runtime CSV parse failure; Code Builder corrected the misaligned `SacredSwordWave` row in `EnemySkillData.csv` and reimported the CSV until the runtime catalog synced again.

## Task: 2026-05-18 NewRunScene Reward Runtime Choice Separation

### Task title

Split NewRunScene offering reward bookkeeping from runtime skill-choice bookkeeping.

### Goals

- Keep enhancement offerings working in `NewRunScene` without relying on reward-id prefix heuristics.
- Ensure runtime skill modifiers consume explicit chosen runtime choice IDs instead of all chosen reward IDs.
- Preserve existing active/passive learn offerings and the current prisoner reward flow.

### Constraints

- Role Owner is Code Builder.
- This slice changed session bookkeeping and offering eligibility only; it did not add new non-Eve modifier rows.
- Runtime combat modifier behavior still depends on `SkillChoiceModifierData.csv` coverage for the linked choice IDs.
- Code Reviewer execution requires explicit user permission and was not run.
...
- If future offering rows should trigger combat modifiers for non-Eve monsters, author explicit `linked_choice_id` + modifier rows instead of overloading reward-only rows.
- If the user wants save/load serialization later, make sure any persistence path stores both `ChosenRewardIds` and `ChosenChoiceIds`.

### Evidence

- `Pakuri/Assets/Scripts2/InGame/UI/InGameUIManager.cs` now sends `rewardId` and `linkedChoiceId` separately into `RunSession.RecordOfferingChoice(...)`.
- The same file now uses `RewardChoiceDefinition.ActiveSkillId` / `PassiveSkillId` for enhancement availability instead of reward-id prefix matching.
- `Pakuri/Assets/Scripts2/InGame/Run/RunSession.cs` now maintains `ChosenChoiceIds` beside `ChosenRewardIds` on both the session and per-monster state.
- `Pakuri/Assets/Scripts2/InGame/Units/UnitFactory.cs` now maps `RunMonsterState.ChosenChoiceIds` into `UnitStateBucket.ChosenChoiceIds`, which is the set read by `SkillChoiceResolver.cs`.
...
- Unity-MCP `manage_scene validate` reported `NewRunScene` clean with `missingScripts=0` and `brokenPrefabs=0`.

### History

- 2026-05-18: User explicitly assigned Code Builder to implement the roadmap-driven CSV choice/reward/modifier cleanup work for the kept new scene flow.

## Task: 2026-05-18 NewRunScene CSV Catalog Authority Update

### Task title

Remove `NewRunScene` runtime dependence on the serialized fallback game-data catalog and rely on the CSV runtime catalog path.

### Goals

- Keep `NewRunScene` unit spawning and enemy combat data resolution on the Scripts2 CSV runtime path.
- Remove the serialized `fallbackCatalog` dependency from the kept new run scene.
- Preserve the current new scene flow while enemy skill runtime values move to CSV-authored ownership.

### Constraints

- Role Owner is Code Builder.
- This slice did not remove the generic fallback-capable helper signatures in unrelated APIs; it removed the actual `NewRunScene` and test-bootstrap dependency.
- Scene/prefab runtime prefab binding for monster units and enemy unit bodies remains the existing authored prefab setup; this task only changed enemy skill effect prefab ownership.
- Code Reviewer execution requires explicit user permission and was not run.
...
- If the user wants the remaining Legacy data assets retired, audit the rest of the project for `GameDataCatalog.asset` references now that `NewRunScene` no longer serializes `fallbackCatalog`.
- If the user wants full enemy-data authority under CSV, follow with passive-multiplier migration and prefab/body-spawn path cleanup.

### Evidence

- `Pakuri/Assets/Scripts2/InGame/Core/NewRunUnitSpawnManager.cs` no longer declares or uses `fallbackCatalog`; catalog resolution now stops at `PakuriDataManager.Instance.CurrentCatalog` or `PakuriCsvRuntimeData.ResolveCatalogOrFallback(null)`.
- `Pakuri/Assets/Scenes/NewScene/NewRunScene.unity` no longer contains the serialized `fallbackCatalog` line that previously referenced `Pakuri/Assets/Legacy/Data/GameData/GameDataCatalog.asset`.
- `Pakuri/Assets/Scripts2/InGame/Core/InGameTestDataManager.cs` no longer serializes `fallbackCatalog` and now bootstraps sample data through the CSV runtime path.
- `Pakuri/Assets/Scripts2/InGame/Core/InGameCombatManager.cs` now resolves enemy skill visual prefabs directly from `EnemyUnitRuntimeModel.ActiveSkillEffectPrefab`, which is populated from CSV runtime data instead of scene-local switch wiring.
- `dotnet build Pakuri\Assembly-CSharp.csproj --no-restore` and `dotnet build Pakuri\Assembly-CSharp-Editor.csproj --no-restore` completed with 0 errors and the existing assembly-reference warnings.
- Unity-MCP loaded `Assets/Scenes/NewScene/NewRunScene.unity`, reported it as the active scene, and `manage_scene validate` returned `totalIssues=0`, `missingScripts=0`, and `brokenPrefabs=0`.
- Unity-MCP warning/error console read after scene validation returned only MCP client handler logs.

### History

- 2026-05-18: User explicitly assigned Code Builder to implement the report-driven fallback catalog removal/replacement work in the kept `NewRunScene` flow.

## Task: 2026-05-17 New Scene Legacy Controller Retirement Phase5-6

### Task title

Delete old-scene-only Legacy runtime scripts and remove every scene outside `Assets/Scenes/NewScene`.

### Goals

- Complete Phase 5 by deleting the old Run/MainMenu/Debug controller cluster and the remaining old Legacy combat runtime scripts.
- Complete Phase 6 by deleting all non-`Assets/Scenes/NewScene` scenes and leaving only the new menu/run scene pair in build settings.
- Preserve the current new scene flow and keep any still-used fallback data assets intact.

### Constraints

- Role Owner is Code Builder.
- User explicitly approved keeping only scenes under `Assets/Scenes/NewScene`; all other scene files were allowed to be deleted.
- `Assets/Legacy/Data` and `Assets/Legacy/CSVdata` were not deleted in this slice because the kept new flow still has a serialized reference to `Legacy/Data/GameData/GameDataCatalog.asset`.
- Code Reviewer execution requires explicit user permission and was not run.
...
Phase 5 and Phase 6 implemented and locally verified.

### Next Actions

- If the user wants the repository fully de-legacyfied, the next safe slice is to migrate or remove the remaining `Assets/Legacy/Data` and `Assets/Legacy/CSVdata` assets after replacing the kept `GameDataCatalog.asset` fallback reference in the new scene flow.
- User verifies in Unity that the project opens with only `NewMainMenu.unity` and `NewRunScene.unity` as the supported scene path.

### Evidence

- Deleted the remaining `Pakuri/Assets/Legacy/Scripts` tree; `Test-Path Pakuri\Assets\Legacy\Scripts` returned `False`.
- Deleted old scene files `Pakuri/Assets/Scenes/DebugScene.unity`, `MainMenuScene.unity`, `NewRunScene.unity`, and `RunScene.unity`; file checks returned `Exists=False` for all four.
- `Get-ChildItem Pakuri\Assets\Scenes -Recurse -Filter *.unity` now returns only `Assets/Scenes/NewScene/NewMainMenu.unity` and `Assets/Scenes/NewScene/NewRunScene.unity`.
- `Pakuri/ProjectSettings/EditorBuildSettings.asset` now contains only `Assets/Scenes/NewScene/NewMainMenu.unity` and `Assets/Scenes/NewScene/NewRunScene.unity`.
- `Select-String -Path Pakuri\Assembly-CSharp.csproj -Pattern 'Assets\\Legacy\\Scripts\\' | Measure-Object` returned `Count=0`, confirming all Legacy script compile includes were removed.
...
- Unity-MCP `manage_scene validate` reported `NewRunScene` clean with `missingScripts=0`, and the warning/error console read returned only MCP client handler logs.
- A GUID scan over kept `Assets/Scenes/NewScene/*.unity` against remaining `Assets/Legacy` meta files found one remaining hit, `Pakuri/Assets/Legacy/Data/GameData/GameDataCatalog.asset.meta`, confirming that the kept new flow still references that Legacy data asset but not the deleted Legacy scripts.

### History

- 2026-05-17: User explicitly assigned Code Builder to continue with Phase 5 through Phase 6 and stated that only scenes under `Assets/Scenes/NewScene` should remain.
