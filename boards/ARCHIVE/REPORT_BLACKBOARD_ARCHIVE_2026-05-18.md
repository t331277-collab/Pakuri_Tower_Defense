...
## Task: 2026-05-18 Stage1 Enemy CSV Skill Runtime Explanation HTML

### Task title

Create an HTML explanation of how Stage1 enemy skills are currently implemented from CSV and scene data.

### Goals

- Explain the current responsibility split between `stage_one_enemies.csv`, `EnemySkillData.csv`, and `EffectManager`.
- Record the new `basic_skill` plus `stage_one_skill` runtime behavior from actual inspected code.
- Show how duplicate skills, support specials, offensive specials, and scene-authored effect prefabs currently work.

### Constraints

- Role Owner is Designer.
- Documentation-only update; no gameplay logic or CSV authoring rule was changed in this task.
- Claims must stay grounded in the current inspected files and Unity-MCP scene inspection.
...
Completed as a new HTML explanation report.

### Next Actions

- Use `Pakuri/reference/Report/2026-05-18-stage1-enemy-csv-skill-runtime-explained.html` when discussing current Stage1 enemy CSV skill behavior instead of re-explaining the flow ad hoc.
- If the runtime policy changes later, refresh the report sections for offensive priority, duplicate-skill collapse, and EffectManager scene mappings.

### Evidence

- Added `Pakuri/reference/Report/2026-05-18-stage1-enemy-csv-skill-runtime-explained.html`.
- The report cites `stage_one_enemies.csv:3-10` for the current `basic_skill` / `stage_one_skill` authored rows.
- The report cites `EnemySkillData.csv:3-10` for the currently active Stage1 skill runtime rows such as `Slash`, `AimedShot`, `ShurikenThrow`, `Heal`, `GuardianFlag`, `ChargeCommand`, and `SacredSwordWave`.
- The report cites `PakuriCsvRuntimeData.EnemyDataset.cs`, `EnemyDefinition.cs`, `EnemyUnitRuntimeModel.cs`, `UnitFactory.cs`, `EnemyCombatSimulationSystem.cs`, and `EffectManager.cs` for the parse/build/runtime/execution path.
- Unity-MCP scene inspection was used as evidence for the current `EffectManager.enemySkillEffects` mappings inside `NewRunScene`.

### History

- 2026-05-18: User asked for an HTML explanation of how enemy skills are currently implemented based on the new CSV structure.

## Task: 2026-05-18 CSV Runtime Roadmap Divergence After EffectManager Migration

### Task title

Record that the 2026-05-18 CSV runtime roadmap is now partially outdated after the `EffectManager` scene-authority migration.

### Goals

- Preserve an evidence-backed note that the roadmap's base monster/enemy skill prefab-path conclusions no longer match the current implementation.
- Keep future reviews from repeating the now-obsolete claim that enemy active skill effect prefabs are runtime-authored from `EnemySkillData.csv`.
- Point report consumers to the new scene-authority implementation evidence before using the roadmap as a current-state document.

### Constraints

- Role Owner is Code Builder.
- This board update documents implementation drift; it does not rewrite the existing HTML roadmap in this slice.
- User-owned Play Mode verification remains outstanding.
...
Recorded as a report-state divergence note.

### Next Actions

- If the user wants the HTML roadmap kept current, update `Pakuri/reference/Report/2026-05-18-csv-runtime-structure-roadmap.html` so it reflects `EffectManager` scene authority and the removal of base-skill prefab-path columns.
- If future report work compares CSV authority again, treat `monster_skill_choices.csv` as the remaining active prefab-path exception and distinguish it from `monster_skills.csv` / `EnemySkillData.csv`.

### Evidence

- `Pakuri/reference/Report/2026-05-18-csv-runtime-structure-roadmap.html` was created before the `EffectManager` migration and still describes base skill effect prefab paths in CSV/runtime authority terms.
- `Pakuri/Assets/Scripts2/InGame/Core/EffectManager.cs` now owns base monster/enemy skill effect prefab lookup and instantiation.
- `Pakuri/Assets/CSVdata/source/monster_skills.csv` and `Pakuri/Assets/CSVdata/EnemySkillData.csv` no longer contain `skill_effect_prefab_path`.
- `Pakuri/Assets/Scenes/NewScene/NewRunScene.unity` now serializes `Assembly-CSharp::Pakuri.InGame.EffectManager`, grouped monster/enemy effect lists, and the combat-manager link to `effectManager`.
- Unity-MCP inspection returned `EffectManager runtimeRoot=RunTimeSkill, monsterGroups=5, enemyGroups=8, combatLink=True`.

### History

- 2026-05-18: Code Builder implemented the `EffectManager` scene-authority migration after the HTML roadmap was authored, making the roadmap partially stale until it is refreshed.

## Task: 2026-05-18 CSV Runtime Structure Roadmap HTML

### Task title

Create an HTML working roadmap for the current CSV runtime structure, issues, and cleanup direction.

### Goals

- Convert the current CSV structure review into a standalone HTML working document.
- Keep the document grounded in inspected Scripts2 runtime code, scene YAML, and active CSV files.
- Add explicit classification for each item: delete, keep, merge, or fix now.

### Constraints

- Role Owner is Designer.
- Documentation-only update; no C# script, scene, prefab, or CSV behavior was changed.
- Claims must distinguish current runtime source files from transition-only or non-runtime CSV files.
...
Completed as a new HTML roadmap.

### Next Actions

- Use `Pakuri/reference/Report/2026-05-18-csv-runtime-structure-roadmap.html` as the working roadmap before changing the CSV source-of-truth structure.
- If implementation starts, treat the report's `즉시 수정 대상` section as the first migration slice.

### Evidence

- Added `Pakuri/reference/Report/2026-05-18-csv-runtime-structure-roadmap.html`.
- The report records `Assets/CSVdata/source/*.csv` and `Assets/CSVdata/EnemySkillData.csv` as current runtime catalog sources based on `PakuriCsvRuntimeData.cs`, `PakuriCsvRuntimeData.Loader.cs`, and `PakuriCsvRuntimeData.Build.cs`.
- The report records `StageDay.csv`, `StageEncounter.csv`, `StageReward.csv`, and `SkillChoiceModifierData.csv` as scene/runtime-linked CSVs based on `NewRunScene.unity`, `NewRunStageManager.cs`, and `InGameCombatManager.cs`.
- The report records that `MonsterStat.csv`, `SkillData.csv`, `EnemyStat.csv`, and `SkillChoiceData.csv` are not directly referenced by the current Scripts2 runtime search, while `EnemySkillData.csv` is.
- The report records hardcoded enemy/monster prefab and skill execution paths based on `NewRunUnitSpawnManager.cs`, `InGameCombatManager.cs`, `EnemyCombatSimulationSystem.cs`, and `InGameSkillDefinitionMapper.cs`.
- The report records the remaining `fallbackCatalog` scene reference to `Legacy/Data/GameData/GameDataCatalog.asset` based on `Assets/Scenes/NewScene/NewRunScene.unity`.

### History

- 2026-05-18: User asked to convert the CSV structure review into an HTML working roadmap and requested delete/keep/merge/fix-now classification.
